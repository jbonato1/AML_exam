...
def trainEnc_MI(stats,model, optimizer, scheduler,dataloaders,device,kwargs):
    """This funcion performs training of DIM. Using use_prior flag it is possible to
    activate the classifier as a regularizer. The discriminator element in the dictionary kwargs
    is the classifier which has to be embedded into the architecture.
    Tensorboard is also available
    """
...
def train_classifier(stats,model, optimizer, scheduler,dataloaders,device,kwargs):
    """
    This funcion performs training of classifier. Tensorboard available using kwargs dictionary
